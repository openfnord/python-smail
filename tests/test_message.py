...
from smail import encrypt_message, sign_and_encrypt_message, sign_message
from smail.message import make_msg
from tests.conftest import FIXTURE_DIR
from tests.fixtures import get_message, get_plain_text_message
...
class TestMessage:
    ...
    def setup_class(cls):
        ...
        try:
            copyfile(
                os.path.join(FIXTURE_DIR, "BobRSASignByCarl_password.p12"),
        ...
        try:
            copyfile(
                os.path.join(FIXTURE_DIR, "BobRSASignByCarl_password.txt"),
    ...
    @pytest.mark.parametrize(
        "output_file_eml,pub_key,private_key,digest_alg,sig_alg,depr",
        [
            ("plain_message_signed_by_alice_md5.eml", "AliceRSASignByCarl.pem", "AlicePrivRSASign.pem", "md5", "rsa",
             True),
            ("plain_message_signed_by_alice_sha1.eml", "AliceRSASignByCarl.pem", "AlicePrivRSASign.pem", "sha1", "rsa",
             True),
            ("plain_message_signed_by_alice_sha1_pss.eml", "AliceRSASignByCarl.pem", "AlicePrivRSASign.pem", "sha1",
             "pss", True),
            ("plain_message_signed_by_alice_sha256.eml", "AliceRSASignByCarl.pem", "AlicePrivRSASign.pem", "sha256",
             "rsa", False),
            (
                    "plain_message_signed_by_alice_sha256_pss.eml",
                    "AliceRSASignByCarl.pem",
                    "AlicePrivRSASign.pem",
                    "sha256",
                    "pss",
                    False,
            ),
    ...
    def test_plain_message_signed_by_alice(self, output_file_eml, pub_key, private_key, digest_alg, sig_alg, depr):
        ...
        signed_message = sign_message(
            msg, key_signer_info, cert_signer, digest_alg=digest_alg, sig_alg=sig_alg, allow_deprecated=depr
        )
    ...
    @pytest.mark.parametrize(
        "output_file_eml,pub_key,private_key,incl_self,incl_ca",
        [
            (
                    "plain_message_signed_by_alice_sha256_incl_false_false.eml",
                    "AliceRSASignByCarl.pem",
    ...
    def test_plain_message_signed_by_alice_includes(self, output_file_eml, pub_key, private_key, incl_self, incl_ca):
        ...
        if incl_ca:
            ...
            signed_message = sign_message(
                msg, key_signer_info, cert_signer, digest_alg="sha256", sig_alg="rsa", include_cert_signer=incl_self
            )
    ...
    @pytest.mark.parametrize(
        "output_file_eml,pub_keys,algorithm",
        [
            ...
            (
                    "plain_message_encrypted_for_alice_and_bob_aes256_cbc.eml",
                    ("AliceRSASignByCarl.pem", "BobRSASignByCarl.pem"),
                    "aes256_cbc",
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "attachments,output_file_eml,pub_keys",
        [
            (["sample1.pdf"], "msg_w1_att_message_encrypted_for_bob_aes256_cbc.eml", ["BobRSASignByCarl.pem"]),
    ...
    def test_make_message(self, addr, name, reci, subject):
        msg = make_msg(
            ...
            subject=subject,
            text="Text Body",
            html="This is a <strong>HTML</strong> body.",
            img_list=None,
            attachments=None,
        )
