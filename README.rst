...
The code below loads Bob's private and public key in PEM format and uses it
to decrypt an encrypted message sent to him the e-mail in S/MIME format::

    import os
    from smail import decrypt_message
    ...
    cert = os.path.join('tests', 'testdata', 'BobRSASignByCarl.pem')
    key = os.path.join('tests', 'testdata', 'BobPrivRSAEncrypt.pem')

    decrypted_message = decrypt_message(message, cert, key)
    print(decrypted_message.as_string())
...
The code below loads Alice's public key in PEM format and uses it to verify
the e-mail in S/MIME format::

    import os
    from email import message_from_string
    from smail import verify_message
    ...
    verified_message = verify_message(message, cert_signer)
    print(verified_message.as_string())
...
The code below loads Bob's private and public key and also Alice's public key in PEM format and uses
it to decrypt and verify the e-mail (from Alice to Bob) in S/MIME format::

    import os
    from smail import decrypt_and_verify_message
    ...
    decrypted_verified_message = decrypt_and_verify_message(message, cert, key, cert_signer, prefix='x-')
    print(decrypted_verified_message)
